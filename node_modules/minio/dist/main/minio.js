...
var Client = (function () {
  ...
  _createClass(Client, [{
    ...
    value: function makeRequestStream(options, stream, sha256sum, statusCode, region, cb) {
      ...
      var _makeRequest = function _makeRequest(e, region) {
        ...
        req.once('finish', function () {
          return pipe.removeAllListeners('error');
        });
      };
    ...
    value: function copyObject(arg1, arg2, arg3, arg4, arg5) {
      ...
      var method = 'PUT';
      this.makeRequest({ method: method, bucketName: bucketName, objectName: objectName, headers: headers }, '', 200, '', function (e, response) {
        if (e) return cb(e);
    ...
    value: function setBucketPolicy(bucketName, objectPrefix, bucketPolicy, cb) {
      ...
      this.makeRequest({ method: method, bucketName: bucketName, query: query }, policyPayload, 204, '', cb);
    }
    ...
    value: function getUploader(bucketName, objectName, contentType, multipart) {
      ...
      var upload = function upload(query, stream, length, sha256sum, md5sum, cb) {
        ...
        _this17.makeRequestStream({ method: method, bucketName: bucketName, objectName: objectName, query: query, headers: headers }, stream, sha256sum, 200, '', function (e, response) {
          if (e) return cb(e);
          var etag = response.headers.etag;
          if (etag) {
            etag = etag.replace(/^\"/, '').replace(/\"$/, '');
    ...
    value: function setBucketNotification(bucketName, config, cb) {
      ...
      this.makeRequest({ method: method, bucketName: bucketName, query: query }, payload, 200, '', cb);
    }
  }, {
...
var CopyConditions = (function () {
  ...
  _createClass(CopyConditions, [{
    key: 'setModified',
    value: function setModified(nativedate) {
      if (!isDate(nativedate)) {
        throw new errors.InvalidDateError('Invalid date : cannot be null');
      }
      this.modified = (0, _moment2['default'])(nativedate).format('YYYY-MM-DDThh:mm:ss.SSS') + 'Z';
    }
  }, {
    key: 'setUnmodified',
    value: function setUnmodified(nativedate) {
      if (!isDate(nativedate)) {
        throw new errors.InvalidDateError('Invalid date : cannot be null');
      }
      this.unmodified = (0, _moment2['default'])(nativedate).format('YYYY-MM-DDThh:mm:ss.SSS') + 'Z';
    }
  }, {
    key: 'setMatchETag',
